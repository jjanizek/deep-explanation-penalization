...
### batch size has to be _significantly_ bumped down
...
dataloaders = {x: torch.utils.data.DataLoader(datasets[x], batch_size=args.batch_size, shuffle=True, num_workers=4) for x in ['train', 'test','val']}
...
def train_model(model,dataloaders, criterion, optimizer, background_dataset, num_epochs=25):
    since = time.time()
    val_acc_history = []
    val_loss_history = []
    train_loss_history = []
...
#                     need to do calc beforehand because we do need the gradients
                    ...
                    # get attributions for the ENTIRE model
                    rel, irrel = cd.cd_vgg_features(torch.zeros_like(inputs),inputs, model, model_type='vgg')
                    # just penalize the magnitude of these attributions for the sake of penalizing something
                    add_loss = torch.abs(rel).sum()
...
model, hist_dict = train_model(model, dataloaders, criterion, optimizer_ft, train_dataset, num_epochs=num_epochs)
